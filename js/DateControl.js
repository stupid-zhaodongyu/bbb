...
{
    if (arguments.length > 2){TSXXLei("对不起！传入本控件的参数太多！");return;}
    if (arguments.length == 0){TSXXLei("对不起！您没有传回本控件任何参数！");return;}
...
{
    if (strYear.match(/\D/)!=null){TSXXLei("年份输入参数不是数字！");return;}
    var m = (strYear) ? strYear : new Date().getFullYear();
    if (m < 1000 || m > 9999) {TSXXLei("年份值不在 1000 到 9999 之间！");return;}
    var n = m - 50;
    if (n < 1000) n = 1000;
...
{
    if (strMonth.match(/\D/)!=null){TSXXLei("月份输入参数不是数字！");return;}
...
function tmpSelectHourInnerHTML(strHour) //小时的下拉框
{
    ...
    if (strHour.match(/\D/)!=null){TSXXLei("小时输入参数不是数字！");return;}
...
function tmpSelectMinuteInnerHTML(strMinute) //分钟的下拉框
{
    ...
    if (strMinute.match(/\D/)!=null){TSXXLei("分钟输入参数不是数字！");return;}
...
function tmpSelectSecondInnerHTML(strSecond) //秒的下拉框
{
    ...
    if (strSecond.match(/\D/)!=null){TSXXLei("分钟输入参数不是数字！");return;}
...
function meizzDayClick(n,ex)    //点击显示框选取日期，主输入函数*************
{
    ...
    else {closeLayer(); TSXXLei("您所要输出的控件对象并不存在！");}
}
